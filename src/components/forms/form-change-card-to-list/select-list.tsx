...
export const SelectList = ({
	...
	return (
		<Select onValueChange={value => setValue("newListId", value)}>
			<SelectTrigger
				className={cn("w-full capitalize", "placeholder:normal-case")}>
			...
			<SelectContent className="w-full">
				<SelectGroup>
					...
					<SelectSeparator />
					{lists.map(({ id, name }) => (
						<SelectItem key={id} value={id} disabled={listName === name}>
							{name}
							{listName === name && (
								<div className="text-muted-foreground">{`(Lista atual)`}</div>
							)}
						</SelectItem>
					))}
				</SelectGroup>
