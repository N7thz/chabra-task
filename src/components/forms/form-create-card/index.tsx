		...
		<>
				...
				<Form {...form}>
					<form
						...
						className={cn(
							...
							tasks.length === 0 ? "grid-cols-1" : "grid-cols-2 gap-4"
						)}>
						<div className="space-y-5 w-full">
							<Label htmlFor="title" className="flex flex-col gap-3">
								<span className="self-start">Título do cartão:</span>
								<Input id="title" {...register("title")} />
							</Label>
							{errors.title && (
								<SpanErrorMessage message={errors.title.message} />
							)}
							<Label htmlFor="cnpj" className="flex flex-col gap-3">
								<span className="self-start">CNPJ da empresa:</span>
							...
							<Label className="flex flex-col gap-3">
								<span className="self-start">Prazo:</span>
								<SelectTerm
									date={term}
							...
							<Label className="flex flex-col gap-3">
								<span className="self-start">Prioridade:</span>
								<SelectPriority
									onValueChange={value =>
							...
							<Label className="flex flex-col gap-3">
								<span className="self-start">Responsaveis:</span>
								<SelectOwners
							...
							<Label htmlFor="description" className="flex flex-col gap-3">
								<span className="self-start">Observações:</span>
								<Textarea
									id="description"
						...
						<ScrollArea className={cn(tasks.length !== 0 && "h-[500px]")}>
							<ScrollBar />
							{tasks.length !== 0 && (
								<div className="size-full space-y-4">
									{tasks.map((task, index) => (
										<TaskItem key={task.id} index={index} remove={remove} />
									))}
								</div>
							)}
						</ScrollArea>
			...
			<CardFooter className="gap-4 justify-end">
				<Button
					...
					{isPending || isSuccess ? <Spinner /> : "Confirmar"}
				</Button>
			</CardFooter>
		</>
	)
