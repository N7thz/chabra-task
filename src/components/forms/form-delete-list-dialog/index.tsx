	...
	const { mutate, isPending, isSuccess } = useMutation({
		...
		onSuccess: list => {
			...
			queryClient.setQueryData<List[]>(["find-many-lists"], oldData => {
				...
				const newLists = oldData.filter(oldList => oldList.id !== list.id)
			...
			<DialogContent className="space-y-4">
				<DialogHeader>
					...
					<DialogDescription className="text-destructive text-lg">
						Tem certeza que deseja excluir essa lista? Os cartões também serão
						excluidos
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
					...
					<Button disabled={isPending || isSuccess} onClick={() => mutate(id)}>
